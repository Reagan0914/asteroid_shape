% Modified version of the original function plotsurf from iso2mesh

function hm=plotsurf2(node,face,varargin)
%
...
if(nargin>=2)
  ...
  if(iscell(face))
    ...
    if(size(face,2)==4)
        hold on;
		h=[];
	    for i=1:length(face(:,4))
            if(size(node,2)==3)
                r = (sin(1.5*pi*face(i,4)+pi+pi/4) + 1)/2.0;
                g = (sin(1.5*pi*face(i,4)+pi+pi/4 + pi/2) + 1)/2.0;
                b = (sin(1.5*pi*face(i,4)+pi+pi/4 + pi) + 1)/2.0;
                h=[h plotasurf(node,face(i,1:3),'facecolor',[r g b],varargin{:})];
            else
                h=[h plotasurf(node,face(i,1:3),varargin{:})];
            end
        end
    else
